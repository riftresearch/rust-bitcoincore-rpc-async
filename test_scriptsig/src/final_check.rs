...
fn main() {
    ...
    // Try to use ScriptSig from v29 namespace (this might fail if it doesn't exist there)
    match std::panic::catch_unwind(|| {
        // This is a compile-time check, but we'll structure it to see if it compiles
        let _script_sig_v29 = json::ScriptSig {
            asm: "test".to_string(),
            hex: "test".to_string(),
        };
        "available"
    }) {
        Ok(_) => println!("✅ ScriptSig also available from corepc_types::v29::ScriptSig"),
        Err(_) => println!("❌ ScriptSig NOT available from corepc_types::v29::ScriptSig"),
    }
